...
public class ShopReportLoadTask extends AsyncTask<Void, Void, Boolean> {
    private String url;
    ...
    public ShopReportLoadTask(String url, List<NameValuePair> parameter, ArrayAdapter mAdapter, Activity activity) {
    ...
    protected Boolean doInBackground(Void... params) {
        ...
        if (json != null) {
            try {
                ...
                if (successjson == 1) {
                    myreports = json.getJSONArray(Shopprodukreport.TAG_SHOPPRODUKREPORT);

                    for (int i = 0; i < myreports.length(); i++) {
                        JSONObject reportObj = myreports.getJSONObject(i);
                        Shopprodukreport newReport = Shopprodukreport.GetShopprodukreportFromJson(reportObj);
